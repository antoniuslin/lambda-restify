...
const fmt = restify_utils_1.createFormattersAndAcceptables();
function createRequestModel(customEventSource) {
    const eventSource = Object.assign({}, sampleEventSource, customEventSource || {});
    return new request_1.default(eventSource, log);
}
function createModel(callback, customEventSource) {
    if (!callback) {
        callback = (error, result) => {
        };
    }
    return new response_1.default(callback, createRequestModel(customEventSource), log, fmt.formatters, fmt.acceptable);
}
describe('Response', () => {
    let model;
    let modelResponse;
    function setupNewModel(customEventSource) {
