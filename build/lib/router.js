...
class Router extends events_1.EventEmitter {
    constructor(options, log) {
        ...
        this.reverse = {};
        this.contentType = [];
        this.name = 'lambdaRestifyRouter';
        assert.object(options, 'options');
        this.cache = new lru_cache_1.default({ max: 100 });
    ...
    find(req, res, callback) {
        ...
        if (cacheVal) {
            ...
            callback(null, cacheVal, restify_utils_1.shallowCopy(cacheVal.params));
            return;
        }
        ...
        if (params && r) {
            ...
            callback(null, cacheVal, restify_utils_1.shallowCopy(params));
            return;
        }
        if (typed) {
        ...
        if (versioned) {
            callback(new errors.InvalidVersionError('%s is not supported by %s %s', req.version() || '?', req.method, req.path()));
            return;
        }
        ...
        for (j = 0; j < urls.length; j++) {
            if (matchURL(new RegExp(urls[j]), req.path())) {
                ...
                callback(new errors.MethodNotAllowedError('%s is not allowed', req.method));
                return;
            }
        }
        // clean up the url in case of potential xss
        // https://github.com/restify/node-restify/issues/1018
        callback(new errors.ResourceNotFoundError('%s does not exist', url.parse(req.url).pathname));
    }
}
...
function compileURL(options) {
    ...
    const _url = url.parse(options.url).pathname;
 ...
 * @returns  {Object}
 */
function matchURL(re, reqPath) {
    let i = 0;
    const result = re.exec(reqPath);
    const params = {};
    const restifyParams = re.restifyParams;
    if (!result) {
        return false;
    }
